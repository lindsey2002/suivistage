...
async function loadEvaluations() {
  const { data } = await apiRequest('/admin/evaluations')
...
function renderEvaluations(evaluations) {
  ...
  if (evaluations.length === 0) {
    tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; color:#64748B;">Aucune évaluation</td></tr>'
    return
  }
  ...
  tbody.innerHTML = evaluations.map(e => `
    <tr>
      ...
      <td>
        <span style="font-size:18px; font-weight:700; color:${e.note >= 10 ? '#22C55E' : '#EF4444'};">
          ${e.note}/20
        </span>
      </td>
